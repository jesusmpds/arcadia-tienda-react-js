...
export default function PaymentModal() {
  ...
  const handlechange = (e, property) => {
    ...
    userInfo
      ? setUserInfo({ ...userInfo, [property]: info })
      : setUserInfo([{ [property]: info, ...userInfo }]);
  };
  ...
  const updateFirestoreStock = async () => {
    const topCollection = db.collection("tops").where(
      firebase.firestore.FieldPath.documentId(),
      "in",
      cartProducts.map(i => i.id)
    );

    const calzasCollection = db.collection("calzas").where(
      firebase.firestore.FieldPath.documentId(),
      "in",
      cartProducts.map(i => i.id)
    );
    ...
    <>
        ...
        <div className="modal-dialog modal-dialog-centered modal-lg">
          <div className="modal-content">
            <div className="modal-header">
              <h3 className="modal-title" id="PaymentModal1Label">
            ...
            <div className="modal-body">
              <form>
                ...
                <div className="mb-3">
                  <label htmlFor="userPhone" className="col-form-label">
                    Telefono:
                  </label>
                  <input
                    className="form-control"
      ...
      >
        <div className="modal-dialog modal-dialog-centered modal-lg">
          <div className="modal-content">
            <div className="modal-header">
              <h3 className="modal-title" id="PaymentModal2Title">
                  ...
                  <div className="container-fluid" key={item.id}>
                    <div className="d-flex border rounded w-100 mb-2">
                      ...
                      <div className="col-6 col-md-8 media-body p-3">
                        ...
                        <h3 className="mt-5">
                          Talla:<p className="d-inline"> {item.talla}</p>
                        </h3>

                        <h3 className="mt-5">
                          Cantidad: <p className="d-inline"> {item.quantity}</p>
                        </h3>
                      </div>
                    </div>
                  </div>
            ...
            <div className="container d-flex justify-content-center" id="precioTotal">
              <h3 className="row text-center">
                TOTAL: <br />
                {Amount} {Amount === 1 ? "producto" : "productos"} $({Price}){" "}
              </h3>
            </div>
            <div className="modal-footer">
              <button
